...
if __name__ == "__main__":
    ...
    num_zero = int((num_boxes * (num_boxes - 1)) / 2) + num_boxes
    ...
    for i, (p0, p1) in enumerate(ordered_pairs):
        ...
        print(f"[{i}] {len(circuits)} circuits")

        if (num_circuits == 1) and (len(list(circuits)[0]) == num_boxes):
            break

        # print(boxes_assigned)

    p0_coords = box_coords[p0]
    p1_coords = box_coords[p1]
    print(p0_coords[0] * p1_coords[0])
